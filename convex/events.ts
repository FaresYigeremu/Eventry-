...
import { processQueueInternal } from "./waitingList";
...
async function getAvailabilityInternal(ctx: QueryCtx | MutationCtx, eventId: Id<"events">) {
  ...
  // Count current active valid offers
  const now = Date.now();
...
export const joinWaitingList = mutation({
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    // FIXED: Instead of ctx.runQuery, call your internal shared function cleanly
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo }) => {
    ...
    try {
      ...
      // FIXED: Use the internal context helper instead of nested mutation call
